...
class TestLoadCertchain:
    """Tests for ServiceFactory._load_certchain."""

    def test_load_retail_certchain(self):
        """Test loading retail (embedded) certchain."""
        # Act
        certchain = ServiceFactory._load_certchain(dev_keys=False)

        # Assert
        assert len(certchain) == 0xA00  # 2560 bytes
        assert isinstance(certchain, bytes)

    def test_load_dev_certchain_from_current_dir(self, tmp_path, monkeypatch):
        """Test loading dev certchain from current directory."""
        # Arrange
        monkeypatch.chdir(tmp_path)
        dev_cert_file = tmp_path / "certchain-dev.bin"

        # Create valid dev certchain (0xA00 bytes with correct MD5)
        import hashlib

        # Create dummy certchain that will have the right hash
        # The correct hash is "d5c3d811a7eb87340aa9f4ab1841b6c4"
        # For testing, we'll create actual data that matches
        certchain_data = b"\x00" * 0xA00
        # We need to find data that matches the hash, but for testing
        # we'll mock the hash check instead
        dev_cert_file.write_bytes(certchain_data)

        # Mock the MD5 hash check to return correct hash
        with patch("hashlib.md5") as mock_md5:
            mock_hash = Mock()
            mock_hash.hexdigest.return_value = "d5c3d811a7eb87340aa9f4ab1841b6c4"
            mock_md5.return_value = mock_hash

            # Act
            certchain = ServiceFactory._load_certchain(dev_keys=True)

        # Assert
        assert len(certchain) == 0xA00
        assert certchain == certchain_data

    def test_load_dev_certchain_from_home_3ds(self, tmp_path, monkeypatch):
        """Test loading dev certchain from ~/.3ds/ directory."""
        # Arrange
        home_3ds = tmp_path / ".3ds"
        home_3ds.mkdir()
        dev_cert_file = home_3ds / "certchain-dev.bin"
        ...
        with patch("os.path.expanduser") as mock_expand:
            mock_expand.return_value = str(dev_cert_file)

            # Mock the MD5 hash check
            with patch("hashlib.md5") as mock_md5:
                mock_hash = Mock()
                mock_hash.hexdigest.return_value = "d5c3d811a7eb87340aa9f4ab1841b6c4"
                mock_md5.return_value = mock_hash
    ...
    def test_load_dev_certchain_raises_error_when_not_found(self):
        """Test that loading dev certchain raises error when file not found."""
        # Act & Assert
        with pytest.raises(FileNotFoundError, match="Dev certchain not found or invalid"):
            ServiceFactory._load_certchain(dev_keys=True)

    def test_load_dev_certchain_raises_error_for_invalid_hash(self, tmp_path, monkeypatch):
        """Test that loading dev certchain raises error for invalid hash."""
        # Arrange
        monkeypatch.chdir(tmp_path)
        dev_cert_file = tmp_path / "certchain-dev.bin"

        # Create certchain with wrong hash
        certchain_data = b"\xff" * 0xA00
        dev_cert_file.write_bytes(certchain_data)
